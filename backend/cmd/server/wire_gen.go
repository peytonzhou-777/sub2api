...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	limitedCreditRepository := repository.NewLimitedCreditRepository(client, db)
	billingCacheService := service.ProvideBillingCacheService(billingCache, userRepository, userSubscriptionRepository, apiKeyRepository, userRPMCache, userGroupRateRepository, configConfig, serviceUserPlatformQuotaRepository, limitedCreditRepository)
	...
	limitedCreditService := service.NewLimitedCreditService(limitedCreditRepository)
	promoService := service.ProvidePromoService(promoCodeRepository, userRepository, billingCacheService, client, apiKeyAuthCacheInvalidator, limitedCreditService)
	...
	securityDepositKeyEligibilityRepository := repository.NewSecurityDepositKeyEligibilityRepository(client, db)
	...
	securityDepositBonusService := service.ProvideSecurityDepositBonusService(db, settingService, securityDepositRepository, apiKeyService, billingCacheService)
	securityDepositService := service.ProvideSecurityDepositService(securityDepositRepository, securityDepositKeyEligibilityRepository, apiKeyService, paymentService, settingService, securityDepositBonusService)
	...
	openAIOAuthService := service.ProvideOpenAIOAuthService(proxyRepository, openAIOAuthClient, privacyClientFactory, openAIAccountPersonaRepository, settingService, secretEncryptor, geminiTokenCache)
	...
	openAIGatewayService := service.NewOpenAIGatewayService(accountRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, httpUpstream, deferredService, openAITokenProvider, grokTokenProvider, modelPricingResolver, channelService, balanceNotifyService, settingService, serviceUserPlatformQuotaRepository, openAIAccountPersonaRepository)
	...
	limitedCreditHandler := handler.NewLimitedCreditHandler(limitedCreditService)
	...
	accountTestService := service.ProvideAccountTestService(accountRepository, openAIAccountPersonaRepository, geminiTokenProvider, claudeTokenProvider, grokTokenProvider, antigravityGatewayService, httpUpstream, configConfig, tlsFingerprintProfileService, openAIGatewayService, openAITokenProvider, settingService, pluginManager)
	crsSyncService := service.NewCRSSyncService(accountRepository, proxyRepository, oAuthService, openAIOAuthService, geminiOAuthService, configConfig)
	accountHandler := admin.ProvideAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, grokQuotaService, settingService)
	...
	settingHandler := handler.ProvideAdminSettingHandler(settingService, emailService, turnstileService, aliyunCaptchaService, opsService, paymentConfigService, paymentService, userAttributeService, notificationEmailService, totpService, userService, authService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, resetRebateHandler, recurringCreditHandler, securityDepositHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	openAIAccountAdmissionQueue := repository.NewOpenAIAccountAdmissionQueue(redisClient)
	openAIAccountAdmissionService := service.NewOpenAIAccountAdmissionService(settingService, openAIAccountAdmissionQueue)
	openAIGatewayHandler := handler.ProvideOpenAIGatewayHandler(openAIGatewayService, pluginManager, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, grokQuotaService, securityDepositService, openAIAccountAdmissionService, configConfig, coordinator)
	...
	accountPoolService := service.ProvideAccountPoolService(accountPoolSource, accountPoolSnapshotCache, concurrencyService, usageLogRepository, accountRepository, openAIAccountPersonaRepository, apiKeyService, settingService, configConfig)
	...
	handlerSecurityDepositHandler := handler.NewSecurityDepositHandler(securityDepositService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, limitedCreditHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, accountPoolHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, handlerSecurityDepositHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(authService, userService, settingService, auditLogService)
	optionalJWTAuthMiddleware := middleware.NewOptionalJWTAuthMiddleware(authService, userService, settingService, auditLogService)
	adminAuthMiddleware := middleware.NewAdminAuthMiddleware(authService, userService, settingService, auditLogService)
	apiKeyAuthMiddleware := middleware.NewAPIKeyAuthMiddlewareWithBillingCache(apiKeyService, subscriptionService, billingCacheService, configConfig)
	...
	recurringCreditRunner := service.ProvideRecurringCreditRunner(recurringCreditService, db)
	securityDepositBonusRunner := service.ProvideSecurityDepositBonusRunner(securityDepositBonusService, db)
	...
	accountPoolReconciler := service.ProvideAccountPoolReconciler(accountPoolService, settingService, opsRepository, configConfig)
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, opsService, opsIngressRejectAggregator, apiKeyService, authCacheInvalidationWorker, schedulerSnapshotService, tokenRefreshService, accountExpiryService, cnProviderBalanceCheckService, openAICodexVersionSyncService, proxyExpiryService, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, batchImageCleanupService, batchImageWorkerRuntime, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, openAIGatewayService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, recurringCreditRunner, securityDepositBonusRunner, channelMonitorRunner, channelMonitorV2Aggregator, userPlatformQuotaUsageFlusher, upstreamBillingProbeService, ollamaCloudUsageService, auditLogService, accountPoolReconciler, openAIQuotaAutoResetService, promptService, pluginManager)
