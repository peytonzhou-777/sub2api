...
func TestOpenAIUserConversationBindingProvisionalLifecycle(t *testing.T) {
	...
	require.WithinDuration(t, time.Now().UTC().Add(config.ConversationActiveTTL()), *loaded.ActiveUntil, 5*time.Second)
	require.WithinDuration(t, time.Now().UTC().Add(config.ConversationIdentityTTL()), loaded.ExpiresAt, 5*time.Second)
