...
// is followed by the local observation fields and native_compaction_v2; created_at is last.
func TestPrepareUsageLogInsert_SessionIDArgWiring(t *testing.T) {
	require.Len(t, usageLogInsertArgTypes, 72, "arg-type table must include turn-state size")
	...
	// created_at 与 native_compaction_v2 位于末尾，session_id 后还有本地观测字段。
...
func TestPrepareUsageLogInsert_SubagentArgWiring(t *testing.T) {
	...
	require.Equal(t, true, prepared.args[len(prepared.args)-9])
	require.Equal(t, "boolean", usageLogInsertArgTypes[len(usageLogInsertArgTypes)-9])
}
