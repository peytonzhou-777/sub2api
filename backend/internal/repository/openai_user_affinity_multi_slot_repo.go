...
func (r *accountRepository) ConvergeOpenAIUserResidentSlots(ctx context.Context, userID int64, scopeKey string, config service.OpenAIUserAffinityConfig, now time.Time) error {
	...
	if _, err := exec.ExecContext(ctx, `
		...
		userID, scopeKey, now, int64(config.ConversationIdentityTTL().Seconds()), int64(config.ConversationActiveTTL().Seconds())); err != nil {
		return err
	}
	// 常驻槽位过期只结束选号偏好，不终止仍活跃的会话。
...
func (r *accountRepository) GetOpenAIUserConversationBinding(ctx context.Context, userID, apiKeyID int64, scopeKey, conversationHash string) (*service.OpenAIUserConversationBinding, error) {
	return r.getOpenAIUserConversationBinding(ctx, `
		...
		WHERE user_id = $1 AND api_key_id = $2 AND scope_key = $3 AND conversation_hash = $4
		  AND binding_epoch = $5 AND status IN ('provisional', 'active', 'draining')
...
func (r *accountRepository) GetOpenAIUserConversationBindingByAlias(ctx context.Context, userID, apiKeyID int64, scopeKey, aliasType, aliasHash string) (*service.OpenAIUserConversationBinding, error) {
	return r.getOpenAIUserConversationBinding(ctx, `
		...
		WHERE a.user_id = $1 AND a.api_key_id = $2 AND a.scope_key = $3
		  AND a.alias_type = $4 AND a.alias_hash = $5 AND (a.expires_at > NOW() OR openai_conversation_has_activity(a.binding_id))
		  AND b.binding_epoch = $6 AND b.status IN ('provisional', 'active', 'draining')
          AND openai_conversation_is_live(b.id, b.active_until, b.expires_at)`,
	...
	err := scanSingleRow(ctx, exec, `
		SELECT EXISTS (
			...
			WHERE b.id = $1 AND b.user_id = $2 AND b.api_key_id = $3 AND b.scope_key = $4
			  ...
			  AND s.status IN ('provisional', 'active', 'replacement_pending', 'draining', 'expired')
	...
	for _, alias := range aliases {
		...
		err := scanSingleRow(ctx, exec, `
			...
			WHERE user_id = $1 AND api_key_id = $2 AND scope_key = $3
			  AND alias_type = $4 AND alias_hash = $5 AND (expires_at > NOW() OR openai_conversation_has_activity(binding_id))
...
func getOpenAIUserConversationBindingByAliasesForUpdate(
	...
	for _, alias := range aliases {
		...
		if !service.OpenAIConversationScopesCompatible(normalizeOpenAIUserAffinityScopeKey(candidate.ScopeKey), bindingScopeKey) {
			continue
		}
...
func (r *accountRepository) ReserveOpenAIUserConversationBinding(ctx context.Context, reservation service.OpenAIUserConversationReservation) (*service.OpenAIUserConversationBinding, bool, error) {
	...
	if existing != nil && existing.BindingEpoch == service.OpenAIConversationBindingEpoch && existing.FirstOutputCommitted {
		var live bool
		if err := scanSingleRow(ctx, exec, `SELECT openai_conversation_is_live(id,active_until,expires_at) AND status IN ('active','draining') FROM openai_user_conversation_bindings WHERE id=$1`, []any{existing.ID}, &live); err != nil {
			return nil, false, err
		}
		if !live {
			return nil, false, service.ErrOpenAIConversationResetRequired
		}
	}
	if existing != nil &&
		...
		if existingValid {
			if err := upsertOpenAIUserConversationAliases(ctx, exec, existing, reservation.UserID,
				reservation.APIKeyID, existing.ScopeKey, aliases, existing.ExpiresAt); err != nil {
				return nil, false, err
			}
			if tx != nil {
				if err := tx.Commit(); err != nil {
	...
	// 外层事务可能继续使用同一仓储；不可替换的 provisional 满槽必须在触达写入前拒绝。
	preferenceLimit := reservation.MaxResidentSlots
	if preferenceLimit < 1 {
		preferenceLimit = 1
	}
	if preferenceLimit > 5 {
		preferenceLimit = 5
	}
	var occupiedPreferences, replaceablePreferences int
	var hasTargetPreference bool
	if err := scanSingleRow(ctx, exec, `SELECT COUNT(*),COUNT(*) FILTER (WHERE status='active'),COALESCE(BOOL_OR(account_id=$3),FALSE)
        FROM openai_user_resident_slots WHERE user_id=$1 AND scope_key=$2
        AND status IN ('provisional','active','replacement_pending') AND expires_at>$4::timestamptz`,
		[]any{reservation.UserID, reservation.ScopeKey, reservation.AccountID, now}, &occupiedPreferences, &replaceablePreferences, &hasTargetPreference); err != nil {
		return nil, false, err
	}
	if reservation.PreferredResidentSlotID <= 0 && !hasTargetPreference && occupiedPreferences >= preferenceLimit && replaceablePreferences == 0 {
	...
	rows, err := exec.QueryContext(ctx, `
		...
		WHERE user_id = $1 AND scope_key = $2
		  AND (status IN ('provisional', 'active', 'replacement_pending', 'draining')
		       OR (id = $3::bigint AND status = 'expired'))
		ORDER BY account_id, generation FOR UPDATE`, reservation.UserID, reservation.ScopeKey, reservation.PreferredResidentSlotID)
	if err != nil {
		return nil, false, err
	}
	...
	for rows.Next() {
		...
		if status != service.OpenAIUserResidentSlotStatusDraining && status != service.OpenAIUserResidentSlotStatusExpired {
			activeSlotCount++
	...
	if slotID == 0 {
		...
		if activeSlotCount >= maxSlots {
			// 新目标已通过资格校验，只替换最冷的稳定偏好，保留其会话引用供续接。
			var victimID int64
			var victimIndex int
			err := scanSingleRow(ctx, exec, `SELECT id,slot_index FROM openai_user_resident_slots
                WHERE user_id=$1 AND scope_key=$2 AND status='active'
                ORDER BY usage_score * POWER(0.5,GREATEST(EXTRACT(EPOCH FROM ($3::timestamptz-score_updated_at)),0)/$4::double precision),
                         last_success_at ASC NULLS FIRST,id FOR UPDATE LIMIT 1`,
				[]any{reservation.UserID, reservation.ScopeKey, now, reservation.Config.ResidentTTL().Seconds()}, &victimID, &victimIndex)
			if errors.Is(err, sql.ErrNoRows) {
				return nil, false, service.ErrOpenAIUserAffinityResidentSlotsFull
			}
			if err != nil {
			...
			if _, err = exec.ExecContext(ctx, `UPDATE openai_user_resident_slots SET status='draining',updated_at=$2::timestamptz WHERE id=$1`, victimID, now); err != nil {
				return nil, false, err
			}
			delete(usedIndexes, victimIndex)
		}
		slotIndex := 1
...
func (r *accountRepository) CommitOpenAIUserConversationBinding(ctx context.Context, transition service.OpenAIUserConversationTransition) (bool, error) {
	...
	if slotGeneration != transition.SlotGeneration ||
		(slotStatus != service.OpenAIUserResidentSlotStatusActive && slotStatus != service.OpenAIUserResidentSlotStatusProvisional && slotStatus != service.OpenAIUserResidentSlotStatusDraining && slotStatus != service.OpenAIUserResidentSlotStatusReset && slotStatus != service.OpenAIUserResidentSlotStatusExpired) ||
	...
	result, err := exec.ExecContext(ctx, `
		...
		now, now.Add(transition.Config.ConversationActiveTTL()), now.Add(transition.Config.ConversationIdentityTTL()),
	...
	if err := syncOpenAIConversationUserLease(ctx, exec, transition.BindingID, now, transition.Config.ConversationActiveTTL()); err != nil {
		return false, err
	}
	increment := 0.0
	if firstCommit {
	...
	slotResult, err := exec.ExecContext(ctx, `
		UPDATE openai_user_resident_slots SET
			status = CASE WHEN status IN ('draining', 'reset', 'expired') THEN status ELSE 'active' END,
			last_success_at = $1,
			expires_at = $2,
		...
		WHERE id = $5 AND account_id = $6 AND generation = $7
		  AND status IN ('provisional', 'active', 'draining', 'reset', 'expired')`, now, now.Add(transition.Config.ResidentTTL()),
	...
	if responseAliasHash := strings.ToLower(strings.TrimSpace(transition.ResponseAliasHash)); len(responseAliasHash) == 64 {
		if _, err := exec.ExecContext(ctx, `
			...
			now.Add(transition.Config.ConversationIdentityTTL()), transition.AccountID); err != nil {
			return false, err
		}
	}
	...
	}, transition.UserID, transition.APIKeyID, transition.ScopeKey, aliases, now.Add(transition.Config.ConversationIdentityTTL())); err != nil {
		return false, err
	}
	if _, err := exec.ExecContext(ctx, `
		UPDATE openai_user_conversation_aliases SET expires_at = $2
		WHERE binding_id = $1`, transition.BindingID, now.Add(transition.Config.ConversationIdentityTTL())); err != nil {
		return false, err
	}
	if firstCommit {
		if _, err := exec.ExecContext(ctx, `
...
func (r *accountRepository) commitOpenAIUserConversationFailover(ctx context.Context, transition service.OpenAIUserConversationTransition) (bool, error) {
	...
	result, err := exec.ExecContext(ctx, `
		...
		now.Add(transition.Config.ConversationIdentityTTL()), transition.BindingID, transition.ProvisionalToken)
	if err != nil {
		return false, err
	}
	affected, err := result.RowsAffected()
	...
	if _, err := exec.ExecContext(ctx, `
		...
		WHERE binding_id = $1`, transition.BindingID, transition.AccountID, now.Add(transition.Config.ConversationIdentityTTL())); err != nil {
		return false, err
	}
	if _, err := exec.ExecContext(ctx, `
