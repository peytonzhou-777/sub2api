...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		sessionScopeHash           sql.NullString
		sessionSourceHash          sql.NullString
		...
		accountPersonaID           sql.NullInt64
		personaProfile             sql.NullString
		upstreamTurnStateSizeBytes sql.NullInt64
		createdAt                  time.Time
	)
	...
	log := &service.UsageLog{
		...
		APIKeyID:                  apiKeyID,
		AccountID:                 accountID,
		PersonaProfile:            strings.TrimSpace(personaProfile.String),
		Model:                     model,
	...
	if accountPersonaID.Valid {
		value := accountPersonaID.Int64
		log.AccountPersonaID = &value
	}
	if upstreamTurnStateSizeBytes.Valid {
		value := int(upstreamTurnStateSizeBytes.Int64)
		log.UpstreamTurnStateSizeBytes = &value
	}
	...
	if sessionScopeHash.Valid {
		log.SessionScopeHash = &sessionScopeHash.String
	}
	if sessionSourceHash.Valid {
		log.SessionSourceHash = &sessionSourceHash.String
	}
