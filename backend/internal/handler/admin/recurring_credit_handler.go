...
// PreviewReissue 返回异常批次按原资格窗口和额度到期时间补发的确认摘要。
func (h *RecurringCreditHandler) PreviewReissue(c *gin.Context) {
	...
	item, err := h.service.PreviewReissue(c.Request.Context(), taskID, batchID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
// Reissue 将异常批次按原资格口径创建为立即执行的补发批次。
func (h *RecurringCreditHandler) Reissue(c *gin.Context) {
	taskID, ok := parseIDParam(c, "id")
	if !ok {
		return
	}
	...
	item, err := h.service.ReissueBatch(c.Request.Context(), taskID, batchID, actor)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
