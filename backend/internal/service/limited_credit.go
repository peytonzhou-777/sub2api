...
// AuthSourceLimitedCreditGranter 定义复用调用方事务的认证来源限时额度发放能力。
type AuthSourceLimitedCreditGranter interface {
	GrantFromAuthSourceSettings(ctx context.Context, userID int64, providerType string, items []DefaultLimitedCreditSetting) ([]LimitedCreditGrant, error)
}
...
// GrantFromAuthSourceSettings 在首次绑定事务内发放认证来源配置的限时额度。
func (s *LimitedCreditService) GrantFromAuthSourceSettings(ctx context.Context, userID int64, providerType string, items []DefaultLimitedCreditSetting) ([]LimitedCreditGrant, error) {
	if s == nil || s.repo == nil {
		return nil, fmt.Errorf("limited credit service is not configured")
	}
	if userID <= 0 {
	...
	grants := make([]LimitedCreditGrant, 0, len(items))
	for _, item := range items {
		created, err := s.repo.CreateGrant(ctx, &LimitedCreditGrant{
			UserID:        userID,
			SourceType:    LimitedCreditSourceAuthSourceDefault,
			InitialAmount: item.Amount,
			ExpiresAt:     grantedAt.AddDate(0, 0, item.ValidityDays),
			Status:        LimitedCreditStatusActive,
			Notes:         fmt.Sprintf("由认证来源 %s 首次绑定自动发放", strings.TrimSpace(providerType)),
		})
		if err != nil {
			return nil, err
		}
