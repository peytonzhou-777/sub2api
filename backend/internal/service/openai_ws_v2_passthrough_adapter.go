	...
	relayResult, relayExit := openaiwsv2.RunEntry(openaiwsv2.EntryInput{
		...
		Options: openaiwsv2.RelayOptions{
			...
			BeforeWriteClient: func(msgType coderws.MessageType, payload []byte, wroteDownstream bool) error {
				...
				if failureEvent && markOpenAIWSCyberPolicy(c, payload) {
					return nil
				}
				if !failureEvent {
					return nil
				...
				isRateLimit := isOpenAIWSRateLimitError(errCodeRaw, errTypeRaw, errMsgRaw) ||
					openAIStreamFailureStatus(payload, errMsgRaw) == http.StatusTooManyRequests
				...
				if !failureAccountSideEffectsApplied {
					...
					failureAccountSideEffectsApplied = s.handleOpenAIWSFailureAccountSideEffects(
						accountFailureCtx, account, capturedSessionModel, handshakeHeaders, payload,
					)
				}
				return nil
			},
...
func markOpenAIWSCyberPolicy(c *gin.Context, payload []byte) bool {
