...
func TestOpenAIAgentIdentityTaskInvalidRetriesExactlyOnce(t *testing.T) {
	...
	_, err := svc.Forward(ctx, c, account, []byte(`{"model":"gpt-5.4","instructions":"Reply OK","input":[],"stream":false}`))
	require.NoError(t, err)
	require.Equal(t, 1, registerCalls)
	require.Len(t, upstream.requests, 2)
	...
	ctx2 := bindOpenAICodexGatewayTestExecution(t, svc, c2, account)
	_, err = svc.Forward(ctx2, c2, account, []byte(`{"model":"gpt-5.4","instructions":"Reply OK","input":[],"stream":false}`))
	require.Error(t, err)
	...
	ctx3 := bindOpenAICodexGatewayTestExecution(t, svc, c3, account)
	_, err = svc.Forward(ctx3, c3, account, []byte(`{"model":"gpt-5.4","instructions":"Reply OK","input":[],"stream":false}`))
	require.NoError(t, err)
...
func TestOpenAIAgentIdentityCompatRoutesRecoverInvalidTaskOnce(t *testing.T) {
	...
	for index, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := tt.call(svc, ctx, c, account, tt.body)
			require.Error(t, err)
			require.Equal(t, 1, registerCalls)
			require.Len(t, upstream.requests, 2)
...
func TestOpenAIAgentIdentityChatRecoveryKeepsAutoDerivedSessionIsolationStable(t *testing.T) {
	...
	_, err := svc.ForwardAsChatCompletions(ctx, c, account, body, "", "gpt-5.4")
	require.Error(t, err)
	require.Len(t, upstream.requests, 2)
