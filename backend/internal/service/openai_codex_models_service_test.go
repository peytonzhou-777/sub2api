...
func (r codexModelsVisibilityAccountRepo) GetByID(_ context.Context, accountID int64) (*Account, error) {
...
func TestFetchCodexModelsManifestDefaultClientVersion(t *testing.T) {
	...
	wantClientVersion := resolveCodexOutboundIdentity("").version
	if gotClientVersion != wantClientVersion {
		t.Errorf("default client_version: got %q, want %q", gotClientVersion, wantClientVersion)
	}
}
...
func TestFetchCodexModelsManifestAPIKeyCustomUpstream(t *testing.T) {
	...
	if gotRequest.Header.Get("User-Agent") != resolveCodexOutboundIdentity("").userAgent {
...
func TestFetchCodexModelsManifestAPIKeyCompleteBodyWithoutUpstreamETagUsesFinalBodyETag(t *testing.T) {
	...
	svc.accountRepo = codexModelsVisibilityAccountRepo{byID: map[int64]*Account{account.ID: account}}
...
func TestFetchCodexModelsManifestAPIKeyCacheSurvivesClientMutation(t *testing.T) {
	...
	s := newCodexModelsAPIKeyTestService(upstream)
	account := newCodexModelsAPIKeyTestAccount("https://upstream.example")
	s.accountRepo = codexModelsVisibilityAccountRepo{byID: map[int64]*Account{account.ID: account}}
...
func TestFetchCodexModelsManifestAPIKeyRejectsOfficialOpenAIModelsEndpoint(t *testing.T) {
	tests := []struct {
		name    string
		baseURL string
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Zero(t, calls.Load(), "official OpenAI API key models endpoint must not be contacted")
		})
	}
}
