...
func TestOpenAIGatewayService_OAuthMessagesBridgeDoesNotInjectDefaultInstructions(t *testing.T) {
	...
	require.Equal(t, promptCacheKey, upstream.lastReq.Header.Get("Session_Id"))
...
func TestOpenAIGatewayService_CodexFingerprintHTTPTransformedHeaderBodyParityAndDefaultCacheKey(t *testing.T) {
	...
	require.Equal(t, wantRequest, upstream.lastReq.Header.Get("x-client-request-id"))
	require.Equal(t, wantWindow, upstream.lastReq.Header.Get("x-codex-window-id"))

	require.Equal(t, wantPromptCache, gjson.GetBytes(upstream.lastBody, "prompt_cache_key").String())
...
func TestOpenAIGatewayService_CodexFingerprintCompactDoesNotRewriteBodyButStagesHeaders(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	fingerprintIDs := stagedCodexFingerprintIDsForAccount(c, account)
	require.NotNil(t, fingerprintIDs)
	require.NotEqual(t, fingerprintIDs.sessionID, gjson.GetBytes(upstream.lastBody, "prompt_cache_key").String())
	...
	require.Equal(t, fingerprintIDs.windowID, upstream.lastReq.Header.Get("x-codex-window-id"))
}

func TestOpenAIGatewayService_CodexFingerprintMessagesBridgeUsesFingerprintPromptCacheKey(t *testing.T) {
	gin.SetMode(gin.TestMode)
