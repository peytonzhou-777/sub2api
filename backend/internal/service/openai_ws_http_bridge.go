...
func (s *OpenAIGatewayService) proxyOpenAIWSHTTPBridgeTurn(
	...
	officialOpenAIResponses := account.Platform == PlatformOpenAI
	bareErrorPending := false
	var bareErrorPayload []byte
	bareErrorMessage := ""
	bareErrorCyberPolicy := false
	...
	finalizeBareError := func() error {
		...
		if !bareErrorCyberPolicy && !failureAccountSideEffectsApplied {
			accountFailureCtx := ctx
			if openAIStreamFailureStatus(bareErrorPayload, bareErrorMessage) == http.StatusTooManyRequests {
			...
			failureAccountSideEffectsApplied = s.handleOpenAIWSFailureAccountSideEffects(
				accountFailureCtx, account, mappedModel, resp.Header, bareErrorPayload,
			)
		}
	...
	for scanner.Scan() {
		...
		if officialOpenAIResponses && bareErrorPending && (eventType == "response.completed" || eventType == "response.done") {
			// 上游可能先发可恢复的裸 error，再发权威成功终态；此时忽略临时错误及其账号副作用。
			bareErrorPending = false
		...
		if eventType == "error" || eventType == "response.failed" {
			cyberPolicyFailure := markOpenAIWSCyberPolicy(c, upstreamMessage)
			...
			if account.Platform != PlatformGrok && !cyberPolicyFailure && !failureAccountSideEffectsApplied {
				if eventType == "response.failed" || (!officialOpenAIResponses && shouldFailover && !requestScopedCapacity) {
					...
					failureAccountSideEffectsApplied = s.handleOpenAIWSFailureAccountSideEffects(
						accountFailureCtx, account, mappedModel, resp.Header, upstreamMessage,
					)
				}
			}
			...
			if eventType == "error" && !officialOpenAIResponses {
				...
				bareErrorCyberPolicy = cyberPolicyFailure
