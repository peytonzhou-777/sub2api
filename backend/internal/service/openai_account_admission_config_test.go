...
func TestDefaultOpenAIAccountAdmissionConfig(t *testing.T) {
	...
	if cfg.DefaultMaxActiveUsersPerPersona != 1 {
		t.Fatalf("default max active users per Persona = %d, want 1", cfg.DefaultMaxActiveUsersPerPersona)
	}
}

func TestOpenAIAccountAdmissionConfigLegacyJSONMigratesSessionLimit(t *testing.T) {
	requireNoField := `{"max_wait_seconds":45,"max_active_client_sessions":2,"default_output_tokens":4096,"jitter_min_ms":100,"jitter_max_ms":500,"max_queue_depth_per_account":100,"interactive_burst":4,"background_aging_seconds":5}`
	cfg, err := decodeOpenAIAccountAdmissionConfig(requireNoField)
	if err != nil {
		t.Fatal(err)
	}
	normalized, err := ValidateOpenAIAccountAdmissionConfig(cfg)
	if err != nil {
		t.Fatal(err)
	}
	if normalized.DefaultMaxActiveUsersPerPersona != 2 {
		t.Fatalf("legacy JSON active users = %d, want 2", normalized.DefaultMaxActiveUsersPerPersona)
	}
}
...
func TestValidateOpenAIAccountAdmissionConfig(t *testing.T) {
	cases := []struct {
		...
		{"active users", func(c *OpenAIAccountAdmissionConfig) { c.DefaultMaxActiveUsersPerPersona = -1 }},
...
func TestOpenAIAccountAdmissionConfigForPersona(t *testing.T) {
	...
	if opencode.DefaultMaxActiveUsersPerPersona != 2 || opencode.RequestsPerMinute != 20 || opencode.TokensPerMinute != 2000 || opencode.MaxQueueDepthPerAccount != 7 {
		t.Fatalf("unexpected OpenCode policy: %+v", opencode)
	}
	strict := cfg.ForPersona(SessionPersonaCodexCLIStrict)
	if strict.DefaultMaxActiveUsersPerPersona != 1 || strict.RequestsPerMinute != 10 || strict.TokensPerMinute != 1000 || strict.MaxQueueDepthPerAccount != cfg.MaxQueueDepthPerAccount {
