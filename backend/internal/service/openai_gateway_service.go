...
type OpenAIForwardResult struct {
	...
	FirstEventMs *int
	// FirstOutputMs 是首次满足 startsClientOutput 的结构性输出耗时。
	FirstOutputMs      *int
...
type OpenAIGatewayService struct {
	...
	openaiCodexTurnStateOrigins         sync.Map // key: API key + client session, value: origin account
...
func (s *OpenAIGatewayService) GetAccessToken(ctx context.Context, account *Account) (string, string, error) {
	...
	if account.IsSchedulerSnapshot {
		authoritative, err := s.resolveAuthoritativeOpenAICredentialAccount(ctx, account)
		if err != nil {
			return "", "", err
		}
