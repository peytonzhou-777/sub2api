...
type RecurringCreditBatchView struct {
	...
	ReissueOfBatchID      *int64     `json:"reissue_of_batch_id,omitempty"`
...
// RecurringCreditReissuePreview 提供按历史批次固定口径补发前的确认摘要。
type RecurringCreditReissuePreview struct {
	SourceBatchID          int64     `json:"source_batch_id"`
	SourceStatus           string    `json:"source_status"`
	SnapshotSourceBatchID  *int64    `json:"snapshot_source_batch_id,omitempty"`
	EligibilityPolicy      string    `json:"eligibility_policy"`
	Amount                 float64   `json:"amount"`
	ExpiresAt              time.Time `json:"expires_at"`
	QualificationStart     time.Time `json:"qualification_start"`
...
type recurringCreditBatchRecord struct {
	ID                    int64
	TaskID                int64
	TaskName              string
	...
	QualificationEnd      time.Time
	QualificationCutoffAt *time.Time
	ConfigVersion         int
...
// PreviewReissue 按原始批次的固定资格窗口和额度到期时间生成补发预览。
func (s *RecurringCreditService) PreviewReissue(ctx context.Context, taskID, batchID int64) (*RecurringCreditReissuePreview, error) {
	source, root, err := s.loadReissueSource(ctx, s.db, taskID, batchID, false)
	if err != nil {
		return nil, err
	}
	if err = validateReissueStatus(source.Status); err != nil {
		return nil, err
	}
	if err = s.ensureNoCompletedReissue(ctx, s.db, root.ID); err != nil {
		return nil, err
	}

	stats, snapshotSourceID, err := s.reissueStats(ctx, s.db, root)
	if err != nil {
		return nil, err
	}
	disclaimer := fmt.Sprintf("将按批次 #%d 的固定资格窗口和额度到期时间立即补发", root.ID)
	if snapshotSourceID != nil {
		disclaimer += fmt.Sprintf("；资格名单复用批次 #%d 已保存的快照，执行时仍会排除已停用或已删除用户", *snapshotSourceID)
	} else {
		disclaimer += "；确认后按该历史窗口生成资格快照，执行时仍会排除已停用或已删除用户"
	}
	return &RecurringCreditReissuePreview{
		SourceBatchID: root.ID, SourceStatus: source.Status, SnapshotSourceBatchID: snapshotSourceID,
		EligibilityPolicy: root.EligibilityPolicy,
		Amount:            root.Amount, ExpiresAt: root.ExpiresAt, QualificationStart: root.QualificationStart, QualificationEnd: root.QualificationEnd,
		ReferenceEligibleCount: stats.EligibleCount, APIActiveCount: stats.APIActiveCount,
		SiteActiveCount: stats.SiteActiveCount, BothActiveCount: stats.BothActiveCount,
		EstimatedTotal: decimal.NewFromFloat(root.Amount).Mul(decimal.NewFromInt(int64(stats.EligibleCount))).InexactFloat64(),
		Disclaimer:     disclaimer,
	}, nil
}

// ReissueBatch 创建一个立即可领取的补发批次，并保持原批次资格口径和额度到期时间不变。
func (s *RecurringCreditService) ReissueBatch(ctx context.Context, taskID, batchID int64, actor RecurringCreditActor) (*RecurringCreditBatchView, error) {
	if s == nil || s.db == nil {
	...
	source, root, err := s.loadReissueSource(ctx, tx, taskID, batchID, true)
	if err != nil {
		return nil, err
	}
	if err = validateReissueStatus(source.Status); err != nil {
	...
	if err = s.ensureNoCompletedReissue(ctx, tx, root.ID); err != nil {
		return nil, err
	}

	_, snapshotSourceID, err := s.reissueStats(ctx, tx, root)
	if err != nil {
		return nil, err
	}
	now := time.Now().UTC()
	cutoff := root.QualificationEnd
	if root.QualificationCutoffAt != nil {
		cutoff = root.QualificationCutoffAt.UTC()
	}
	...
	if root.EligibilityPolicy == RecurringCreditEligibilityRollingActivity {
		if snapshotSourceID != nil {
			_, err = tx.ExecContext(ctx, copyRollingActivitySnapshotSQL, *snapshotSourceID, newBatchID)
		} else {
			_, err = tx.ExecContext(ctx, rollingActivitySnapshotSQL, cutoff, newBatchID)
		}
		if err != nil {
			return nil, err
		}
		...
		err = tx.QueryRowContext(ctx, `SELECT COUNT(*),
			...
			FROM recurring_credit_user_items WHERE batch_id=$1`, newBatchID).
			Scan(&eligibleCount, &apiCount, &siteCount, &bothCount)
		if err != nil {
			return nil, err
		}
		_, err = tx.ExecContext(ctx, `UPDATE recurring_credit_batches
			SET eligible_user_count=$2,api_active_count=$3,site_active_count=$4,both_active_count=$5,snapshot_completed_at=$6,updated_at=NOW()
			WHERE id=$1`, newBatchID, eligibleCount, apiCount, siteCount, bothCount, now)
		if err != nil {
			return nil, err
		}
	}
	...
	beforeJSON, _ := json.Marshal(map[string]any{"source_batch_id": root.ID, "source_status": source.Status})
	afterJSON, _ := json.Marshal(map[string]any{"reissue_batch_id": newBatchID, "expires_at": root.ExpiresAt, "qualification_start": root.QualificationStart, "qualification_end": root.QualificationEnd})
	_, err = tx.ExecContext(ctx, `INSERT INTO recurring_credit_task_audits(task_id,admin_id,admin_email,client_ip,action,before_snapshot,after_snapshot)
		VALUES($1,$2,$3,$4,'reissue_batch',$5,$6)`, root.TaskID, actor.AdminID, email, actor.IP, string(beforeJSON), string(afterJSON))
	if err != nil {
		return nil, err
	}
	...
	s.notifyRunner()
	return s.getBatch(ctx, newBatchID)
}
...
type recurringCreditQueryer interface {
	QueryRowContext(context.Context, string, ...any) *sql.Row
}

func validateReissueStatus(status string) error {
	switch status {
	case "skipped", "missed", "failed":
		return nil
	case "running":
		return infraerrors.New(http.StatusConflict, "RECURRING_CREDIT_BATCH_STILL_RUNNING", "running batches cannot be reissued")
	default:
		return infraerrors.New(http.StatusConflict, "RECURRING_CREDIT_BATCH_NOT_REISSUABLE", "only skipped, missed or failed batches can be reissued")
	}
}

func (s *RecurringCreditService) loadReissueSource(ctx context.Context, queryer recurringCreditQueryer, taskID, batchID int64, lockRoot bool) (*recurringCreditBatchRecord, *recurringCreditBatchRecord, error) {
	source, err := queryRecurringCreditBatchRecord(ctx, queryer, taskID, batchID, false)
	if err != nil {
		return nil, nil, taskQueryError(err)
	}
	rootID := source.ID
	if source.ReissueOfBatchID != nil {
		rootID = *source.ReissueOfBatchID
	}
	root, err := queryRecurringCreditBatchRecord(ctx, queryer, taskID, rootID, lockRoot)
	if err != nil {
		return nil, nil, taskQueryError(err)
	}
...
func queryRecurringCreditBatchRecord(ctx context.Context, queryer recurringCreditQueryer, taskID, batchID int64, lock bool) (*recurringCreditBatchRecord, error) {
	suffix := ""
	if lock {
		suffix = " FOR UPDATE"
	}
	...
	if cutoff.Valid {
		value := cutoff.Time.UTC()
		record.QualificationCutoffAt = &value
	}
	if snapshotCompleted.Valid {
		value := snapshotCompleted.Time.UTC()
		record.SnapshotCompletedAt = &value
	}
	if validity.Valid {
	...
	if reissueOf.Valid {
		value := reissueOf.Int64
		record.ReissueOfBatchID = &value
	}
	return &record, err
}

func queryActiveReissueID(ctx context.Context, queryer recurringCreditQueryer, rootBatchID int64) (int64, error) {
	var id int64
	err := queryer.QueryRowContext(ctx, `SELECT id FROM recurring_credit_batches
		WHERE reissue_of_batch_id=$1 AND status IN ('running','succeeded','empty')
		ORDER BY created_at DESC LIMIT 1`, rootBatchID).Scan(&id)
	return id, err
}

func (s *RecurringCreditService) ensureNoCompletedReissue(ctx context.Context, queryer recurringCreditQueryer, rootBatchID int64) error {
	id, err := queryActiveReissueID(ctx, queryer, rootBatchID)
	...
	return infraerrors.New(http.StatusConflict, "RECURRING_CREDIT_BATCH_ALREADY_REISSUED", fmt.Sprintf("batch already has reissue #%d", id))
}

func (s *RecurringCreditService) reissueStats(ctx context.Context, queryer recurringCreditQueryer, root *recurringCreditBatchRecord) (recurringCreditActivityStats, *int64, error) {
	if root.EligibilityPolicy == RecurringCreditEligibilityRollingActivity {
		var snapshotBatchID int64
		var stats recurringCreditActivityStats
		err := queryer.QueryRowContext(ctx, `SELECT id,eligible_user_count,api_active_count,site_active_count,both_active_count
			FROM recurring_credit_batches
			WHERE (id=$1 OR reissue_of_batch_id=$1) AND snapshot_completed_at IS NOT NULL
			ORDER BY CASE WHEN id=$1 THEN 0 ELSE 1 END,created_at,id LIMIT 1`, root.ID).
			Scan(&snapshotBatchID, &stats.EligibleCount, &stats.APIActiveCount, &stats.SiteActiveCount, &stats.BothActiveCount)
		if err == nil {
			return stats, &snapshotBatchID, nil
		}
		if err != sql.ErrNoRows {
		...
		cutoff := root.QualificationEnd
		if root.QualificationCutoffAt != nil {
			cutoff = *root.QualificationCutoffAt
		}
		err = queryer.QueryRowContext(ctx, rollingActivityStatsSQL, cutoff).Scan(
			&stats.EligibleCount,
	...
	stats, err := legacyReissueStats(ctx, queryer, root)
	return stats, nil, err
}

func legacyReissueStats(ctx context.Context, queryer recurringCreditQueryer, root *recurringCreditBatchRecord) (recurringCreditActivityStats, error) {
	var stats recurringCreditActivityStats
	cutoff := root.QualificationEnd
	if root.QualificationCutoffAt != nil {
		cutoff = *root.QualificationCutoffAt
	}
	if root.ScheduleType == RecurringCreditImmediate {
		err := queryer.QueryRowContext(ctx, `SELECT COUNT(*) FROM users
			WHERE created_at <= $1 AND (deleted_at IS NULL OR deleted_at > $1)`, cutoff).Scan(&stats.EligibleCount)
		return stats, err
	}
	err := queryer.QueryRowContext(ctx, `WITH usage AS (
			SELECT user_id FROM usage_logs WHERE created_at >= $1 AND created_at < $2 AND created_at <= $3
			GROUP BY user_id HAVING SUM(actual_cost)>0
		), recharge AS (
			SELECT user_id FROM payment_orders
			WHERE order_type='balance' AND completed_at >= $1 AND completed_at < $2 AND completed_at <= $3
			GROUP BY user_id
			HAVING SUM(GREATEST(amount-CASE WHEN refund_at IS NOT NULL AND refund_at <= $3 THEN refund_amount ELSE 0 END,0))>0
		), ids AS (SELECT user_id FROM usage UNION SELECT user_id FROM recharge)
		SELECT COUNT(*) FROM ids`, root.QualificationStart, root.QualificationEnd, cutoff).Scan(&stats.EligibleCount)
	return stats, err
}
...
const rollingActivityCandidatesSQL = `WITH api_activity AS (
	...
	FROM api_keys
	WHERE last_used_at >= $1::timestamptz - INTERVAL '30 days 1 minute' AND last_used_at < $1::timestamptz
	GROUP BY user_id
), site_activity AS (
	SELECT id AS user_id,last_active_at AS site_last_active_at
	FROM users
	WHERE status='active' AND deleted_at IS NULL
		AND last_active_at >= $1::timestamptz - INTERVAL '30 days' AND last_active_at < $1::timestamptz
), candidates AS (
...
const copyRollingActivitySnapshotSQL = `INSERT INTO recurring_credit_user_items(
...
SELECT $2,user_id,email,username,user_status,user_deleted,actual_cost,net_recharge,
	api_last_used_at,site_last_active_at,qualification_reason,0,'pending',''
...
const batchSelectColumns = `id,task_id,task_name,reissue_of_batch_id,
	(status IN ('skipped','missed','failed') AND NOT EXISTS (
		SELECT 1 FROM recurring_credit_batches reissue
