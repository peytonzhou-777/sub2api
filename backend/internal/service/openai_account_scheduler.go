...
const (
	...
	openAIAccountScheduleLayerSameAccountRetry = "same_account_retry"
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	accountCapacity := s.service.effectiveOpenAIAccountAdmissionCapacity(ctx, account)
	if accountCapacity <= 0 {
		_ = s.service.deleteStickySessionAccountID(ctx, req.GroupID, sessionHash)
		return nil, false, nil
	}
	result, acquireErr := s.service.tryAcquireAccountSlot(ctx, accountID, accountCapacity)
	...
	if s.service.concurrencyService != nil {
		...
		return attachSelectionProfitGate(ctx, &AccountSelectionResult{
			...
			WaitPlan: &AccountWaitPlan{
				AccountID:      accountID,
				MaxConcurrency: accountCapacity,
...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAISelectionOrderWithBudget(
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		candidateCapacity := s.service.effectiveOpenAIAccountAdmissionCapacity(ctx, candidate.account)
		if candidateCapacity <= 0 {
			continue
		}
		if candidate.loadKnown && candidateCapacity > 0 &&
			candidate.loadInfo.CurrentConcurrency >= candidateCapacity {
			continue
		}

		result, attempted, acquireErr := s.tryAcquireOpenAIAccountSlot(ctx, candidate.account.ID, candidateCapacity, budget)
		if !attempted {
			break
		}
		if acquireErr != nil {
		...
		freshCapacity := s.service.effectiveOpenAIAccountAdmissionCapacity(ctx, fresh)
		if freshCapacity != candidateCapacity {
			release(result)
			result, attempted, acquireErr = s.tryAcquireOpenAIAccountSlot(ctx, fresh.ID, freshCapacity, budget)
			if !attempted {
				continue
			}
			if acquireErr != nil {
...
func (s *defaultOpenAIAccountScheduler) tryFallbackToWeightedSticky(
	...
	for _, accountID := range []int64{req.StickyPreviousAccountID, req.StickyAccountID} {
		...
		accountCapacity := s.service.effectiveOpenAIAccountAdmissionCapacity(ctx, account)
		result, acquireErr := s.service.tryAcquireAccountSlot(ctx, account.ID, accountCapacity)
		if acquireErr != nil {
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	for i := range accounts {
		...
		accountCapacity := s.service.effectiveOpenAIAccountAdmissionCapacity(ctx, account)
		if accountCapacity <= 0 {
			filterStats.exclude("persona_capacity_unavailable")
			continue
		}
...
func (s *defaultOpenAIAccountScheduler) finishLoadBalanceSelectionFallback(
	...
	for pass := 0; pass < passes; pass++ {
		...
		for _, candidate := range attempt.selectionOrder {
			...
			if budget != nil && budget.limited {
				candidateCapacity := s.service.effectiveOpenAIAccountAdmissionCapacity(ctx, candidate.account)
				knownFull := candidate.loadKnown && candidateCapacity > 0 &&
					candidate.loadInfo.CurrentConcurrency >= candidateCapacity
			...
			return attachSelectionProfitGate(ctx, &AccountSelectionResult{
				...
				WaitPlan: &AccountWaitPlan{
					AccountID:      fresh.ID,
					MaxConcurrency: s.service.effectiveOpenAIAccountAdmissionCapacity(ctx, fresh),
...
// ObserveOpenAIAccountHealthFailure 记录已写出响应后无法进入调度结果路径的账号健康失败。
