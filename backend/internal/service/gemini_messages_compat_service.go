...
func (s *GeminiMessagesCompatService) hydrateSelectedAccount(ctx context.Context, account *Account) (*Account, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("load selected gemini account %d from authoritative repository: %w", account.ID, err)
	}
	if hydrated == nil {
		return nil, fmt.Errorf("selected gemini account %d not found in authoritative repository", account.ID)
	}
	return hydrated, nil
}
