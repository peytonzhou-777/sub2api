...
func TestOpenAIWSStateStoreConnectionTargetIsolation(t *testing.T) {
	...
	personaProxy := newOpenAIWSConnectionTargetWithProxy(accountA, OpenAIUpstreamTransportResponsesWebsocketV2, "wss://api.openai.com/v1/responses", headersA, "http://persona-proxy.example:8080")
	personaDirect := newOpenAIWSConnectionTargetWithProxy(accountA, OpenAIUpstreamTransportResponsesWebsocketV2, "wss://api.openai.com/v1/responses", headersA, "")
	...
	require.False(t, ok)
	_, ok = getOpenAIWSResponseConn(store, "resp_target", personaProxy)
	require.False(t, ok)
	_, ok = getOpenAIWSResponseConn(store, "resp_target", personaDirect)
	require.False(t, ok)
