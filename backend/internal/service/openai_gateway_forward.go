...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if instructionsEmpty && account.UsesOpenAICodexProtocol() && !compatMessagesBridge && !nativeCNResponses && !isOpenCodePersona {
	...
	if account.UsesOpenAICodexProtocol() && !isOpenCodePersona {
		...
		// 账号身份命名空间只补充本地指纹未覆盖的字段，随后统一复用
		// 本地指纹解析结果，避免 failover 时出现两套随机标识。
		...
		// 指纹收敛：所有 Responses 入口统一准备出站头；legacy compact 的
		// body 形态特殊，只应用头部，不注入 client_metadata。
		fpIDs, fpErr := s.prepareCodexFingerprintForAttempt(ctx, c, account, body, false)
		if fpErr != nil {
			return nil, fpErr
		}
