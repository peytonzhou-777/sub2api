...
func TestOpenAIGatewayService_Forward_WSv2_CodexFingerprintHandshakeBodyParityAndDefaultCacheKey(t *testing.T) {
	...
	require.Equal(t, wantRequest, captureDialer.lastHeaders.Get("x-client-request-id"))
	require.Equal(t, wantWindow, captureDialer.lastHeaders.Get("x-codex-window-id"))

	require.Equal(t, wantPromptCache, gjson.Get(payloadJSON, "prompt_cache_key").String())
...
func TestOpenAIGatewayService_Forward_WSv2StoreFalseDisableForceNewConnStillIsolatesSessions(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, result2)
	require.Equal(t, int64(2), upgradeCount.Load(), "关闭强制新连只允许同 session 被动复用，不得跨 session 共享有状态连接")
}
