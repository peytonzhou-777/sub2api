...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	if account != nil && account.UsesOpenAICodexProtocol() {
		...
		fingerprintedBody, fpErr := s.applyCodexFingerprintForAttempt(
			ctx, c, account, body, false, !isOpenAIResponsesCompactPath(c),
		)
		if fpErr != nil {
			return nil, fpErr
		}
	...
	for {
		...
		if reqStream {
			result, handleErr := s.handleStreamingResponsePassthrough(ctx, resp, c, account, upstreamStart, reqModel, upstreamPassthroughModel)
			if handleErr != nil {
	...
	logOpenAITiming(ctx, c, account, reqModel, reqStream, forwardResult, time.Now())
...
func (s *OpenAIGatewayService) buildUpstreamRequestOpenAIPassthrough(
	...
	if account.Type == AccountTypeOAuth {
		...
		if outboundErr != nil {
			return nil, fmt.Errorf("prepare Codex outbound passthrough request: %w", outboundErr)
		}
	}
	...
	if snapshot := stagedCodexOutboundSnapshot(c, account); snapshot != nil {
		logOpenAIRoutingDiagnostics(ctx, account, "http_passthrough", snapshot.model, snapshot.serviceTier,
...
func (s *OpenAIGatewayService) handleFailoverErrorResponsePassthrough(
	...
	CaptureOpsUpstreamResponse(c, resp.Header, body)
...
func openAIStreamFailedEventRetryableOnSameAccount(account *Account, payload []byte, message string) bool {
	...
	semanticStatus := openAIStreamFailedEventSemanticStatus(payload, message)
	// 流内 429 发生在任何语义输出写出之前时可以安全重放。先耗尽同账号预算，
	// 再由额度/并发分类限制跨账号次数，避免安全重放直接绕过本地高粘性调度。
	if semanticStatus == http.StatusTooManyRequests {
		return true
	}
...
func (s *OpenAIGatewayService) newOpenAIStreamFailoverErrorWithModel(
	...
	statusCode := openAIStreamFailureStatus(payload, message)
	// 流内 429 遵循本地高粘性重试策略，不在此处写入账号级限流/封禁状态；
	// Spark 的独立配额仍写入模型级限流，其他语义失败继续更新账号健康。
	shouldDisable := false
	if statusCode != http.StatusTooManyRequests || (isCodexSparkModel(canonicalModel) && isOpenAIOAuthAccount(account)) {
		statusCode, shouldDisable = s.handleOpenAIStreamTerminalAccountSideEffects(c, account, payload, message, headers, canonicalModel)
	}
	CaptureOpsUpstreamResponse(c, headers, payload)
	...
	// HTTP 200 的外层配额头不能参与普通流内 429 分类，但仍保留在故障对象中供诊断。
	...
	// 未分类流错误继续使用稳定的通用错误体，避免向客户端泄露上游载荷。
