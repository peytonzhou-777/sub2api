...
func (r *RecurringCreditRunner) failExpiredRunning(ctx context.Context) {
	_, _ = r.db.ExecContext(ctx, `WITH failed AS (
			UPDATE recurring_credit_batches SET status='failed',failure_code='EXECUTION_TIMEOUT',failure_message='execution exceeded 30 minutes or automatic attempts were exhausted',finished_at=NOW(),lease_expires_at=NULL,updated_at=NOW() WHERE status='running' AND (claimed_at < NOW()-INTERVAL '30 minutes' OR (attempt_count >= $1 AND lease_expires_at < NOW())) RETURNING task_id,schedule_type,reissue_of_batch_id
		) UPDATE recurring_credit_tasks SET status='completed',remaining_runs=0,next_run_at=NULL,version=version+1,updated_at=NOW() WHERE id IN (SELECT task_id FROM failed WHERE schedule_type='immediate' AND reissue_of_batch_id IS NULL) AND status<>'deleted'`, recurringCreditMaxAttempts)
}
...
func (r *RecurringCreditRunner) executeBatch(ctx context.Context, claim recurringClaim) ([]int64, error) {
	...
	var reissueOf sql.NullInt64
	err = tx.QueryRowContext(ctx, `SELECT task_id,task_name,status,lease_owner,qualification_start,qualification_end,qualification_cutoff_at,expires_at,amount,execution_mode,schedule_type,eligibility_policy,eligible_user_count,reissue_of_batch_id FROM recurring_credit_batches WHERE id=$1`, claim.BatchID).Scan(&taskID, &taskName, &status, &owner, &start, &end, &cutoff, &expires, &amount, &batchMode, &batchSchedule, &eligibilityPolicy, &snapshotEligibleCount, &reissueOf)
	if err != nil {
		return nil, err
	}
	...
	for _, candidate := range candidates {
		...
		if reissueOf.Valid {
			notes = fmt.Sprintf("赠额任务：%s（补发批次 #%d）", taskName, reissueOf.Int64)
		}
		if eligibilityPolicy == RecurringCreditEligibilityRollingActivity {
	...
	if batchMode == RecurringCreditModeFinite && !reissueOf.Valid {
...
func (r *RecurringCreditRunner) markFailed(claim recurringClaim, code, message string) {
	_, _ = r.db.ExecContext(context.Background(), `WITH failed AS (
			UPDATE recurring_credit_batches SET status='failed',failure_code=$3,failure_message=$4,finished_at=NOW(),lease_expires_at=NULL,updated_at=NOW() WHERE id=$1 AND status='running' AND lease_owner=$2 RETURNING task_id,schedule_type,reissue_of_batch_id
		) UPDATE recurring_credit_tasks SET status='completed',remaining_runs=0,next_run_at=NULL,version=version+1,updated_at=NOW() WHERE id IN (SELECT task_id FROM failed WHERE schedule_type='immediate' AND reissue_of_batch_id IS NULL) AND status<>'deleted'`, claim.BatchID, claim.Owner, code, message)
