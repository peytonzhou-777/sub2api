...
func TestStreamFailedEventSafe429RetriesOnSameAccountBeforeSwitch(t *testing.T) {
	nonPool := &Account{ID: 2, Platform: PlatformOpenAI, Type: AccountTypeOAuth}
	for name, payload := range map[string][]byte{
		"rpm":         []byte(`{"type":"response.failed","response":{"error":{"code":"rate_limit_exceeded","message":"requests per minute exceeded"}}}`),
		"concurrency": []byte(`{"type":"response.failed","response":{"error":{"code":"rate_limit_exceeded","message":"Concurrency limit exceeded for account"}}}`),
		"usage":       []byte(`{"type":"response.failed","response":{"error":{"type":"rate_limit_error","code":"usage_limit_reached","message":"usage limit reached"}}}`),
	} {
		t.Run(name, func(t *testing.T) {
			require.True(t, openAIStreamFailedEventRetryableOnSameAccount(nonPool, payload, ""))
		})
	}
}
