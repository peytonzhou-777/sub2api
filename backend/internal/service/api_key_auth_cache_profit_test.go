...
func profitAuthTestAPIKey() *APIKey {
	...
	return &APIKey{
		...
		Group: &Group{
			...
			SecurityDepositPolicyVersion:     "deposit-v1",
		},
	}
}
...
func TestAPIKeyAuthSnapshotProfitControlRoundtrip(t *testing.T) {
	...
	require.Equal(t, 23, snapshot.Version, "v23 起认证快照同时携带本地分组策略与上游 Fast/reasoning 字段")
	...
	require.Equal(t, int64(10000), materialized.Group.SecurityDepositBaseRequiredCents)
	require.Equal(t, "deposit-v1", materialized.Group.SecurityDepositPolicyVersion)
