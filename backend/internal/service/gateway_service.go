...
type AccountSelectionResult struct {
	...
	// accountPermitReleaseFunc 仅释放调度器预抢的账号并发 permit。Persona 用户占用
	// 拥有独立生命周期，统一准入交接时不得随 permit 回滚。
	...
	personaUserReservation *openAIPersonaUserReservationState
...
// ReleaseAccountPermit 仅执行账号并发 permit 释放，不改变 Persona 用户占用。
...
// RollbackOpenAIPersonaUserReservation 放弃尚未被上游接受的 Persona 用户预留。
func (r *AccountSelectionResult) RollbackOpenAIPersonaUserReservation() {
	if r == nil || r.personaUserReservation == nil {
		return
	}
	r.personaUserReservation.rollback()
}
...
type ForwardResult struct {
	...
	// UpstreamResponseServiceTier 是上游实际使用的档位；空值表示未声明。
	UpstreamResponseServiceTier string
	Stream                      bool
	Duration                    time.Duration
	FirstTokenMs                *int // 首字时间（流式请求）
	FirstResponseMs             *int // 首响应阶段耗时
	FirstEventMs                *int // 首事件阶段耗时
	FirstOutputMs               *int // 首结构性输出阶段耗时
