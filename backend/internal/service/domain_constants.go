...
const (
	...
	AffiliateRebateCreditTypePermanent   = "permanent"
	AffiliateRebateCreditTypeLimited     = "limited"
	AffiliateRebateCreditTypeDefault     = AffiliateRebateCreditTypePermanent
	AffiliateRebateCreditValidityDefault = 30
)
...
const (
	...
	RedeemTypeLimitedCredit    = domain.RedeemTypeLimitedCredit
...
const (
	...
	SettingKeyRegistrationUserLimit            = "registration_user_limit"             // 未删除用户绝对上限（0=不限）
	...
	SettingKeyLegacyInvitationExemptionEnabled    = "legacy_invitation_exemption_enabled"   // 是否允许符合条件的老用户免邀请码
	...
	SettingKeyAffiliateRebateCreditType           = "affiliate_rebate_credit_type"          // 返利转入额度类型：permanent / limited
	SettingKeyAffiliateRebateCreditValidityDays   = "affiliate_rebate_credit_validity_days" // 限时返利额度有效期（天）
...
// SettingKeyAuthSourceLimitedCredits 返回指定认证来源的默认限时额度设置键。
func SettingKeyAuthSourceLimitedCredits(source string) string {
	return fmt.Sprintf("auth_source_default_%s_limited_credits", source)
}
