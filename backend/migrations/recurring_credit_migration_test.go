...
func TestMigration187AddsRecurringCreditReissueProtection(t *testing.T) {
	content, err := FS.ReadFile("187_recurring_credit_reissue.sql")
	require.NoError(t, err)
	sql := string(content)
	require.Contains(t, sql, "ADD COLUMN IF NOT EXISTS reissue_of_batch_id")
	require.Contains(t, sql, "REFERENCES recurring_credit_batches(id)")
	require.Contains(t, sql, "idx_recurring_credit_batches_active_reissue")
	require.Contains(t, sql, "status IN ('running', 'succeeded', 'empty')")
}
