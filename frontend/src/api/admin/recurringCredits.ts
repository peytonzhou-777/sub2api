...
export interface RecurringCreditReissuePreview {
  source_batch_id: number
  source_status: RecurringCreditBatchStatus
  snapshot_source_batch_id?: number
  eligibility_policy: 'period_usage_or_recharge' | 'rolling_30d_activity_v1'
  amount: number
  expires_at: string
  qualification_start: string
...
export async function previewRecurringCreditReissue(taskId: number, batchId: number) {
  const { data } = await apiClient.post<RecurringCreditReissuePreview>(`${base}/${taskId}/batches/${batchId}/reissue/preview`)
  return data
}

export async function reissueRecurringCreditBatch(taskId: number, batchId: number) {
  const { data } = await apiClient.post<RecurringCreditBatch>(`${base}/${taskId}/batches/${batchId}/reissue`)
  return data
}
...
export default { previewRecurringCredit, createRecurringCredit, listRecurringCredits, getRecurringCredit, updateRecurringCredit, recurringCreditAction, deleteRecurringCredit, listRecurringCreditBatches, previewRecurringCreditReissue, reissueRecurringCreditBatch, listRecurringCreditUsers, exportRecurringCreditUsers }
