<template>
  <div class="flex flex-col gap-0.5">
    <!-- 并发槽位 -->
    <CapacityBadge :color-class="concurrencyClass" :current="currentConcurrency" :max="effectiveConcurrency">
...
const effectiveConcurrency = computed(() => props.account.effective_concurrency ?? props.account.concurrency)

const concurrencyClass = computed(() => {
  const current = currentConcurrency.value
  const max = effectiveConcurrency.value
