<template>
  <AppLayout>
            ...
            <tbody>
              <tr v-for="batch in batches" :key="batch.id">
                <td class="table-cell whitespace-nowrap">
                  ...
                  <p v-if="batch.reissue_of_batch_id" class="text-xs font-medium text-amber-700 dark:text-amber-300">补发自批次 #{{ batch.reissue_of_batch_id }}</p>
                ...
                <td class="table-cell"><div class="flex justify-center gap-1"><template v-if="['succeeded','empty'].includes(batch.status)"><button class="btn btn-secondary btn-sm" @click="openBatch(batch)">明细</button><button class="btn btn-secondary btn-sm" @click="downloadCSV(batch)">CSV</button></template><button v-if="batch.can_reissue" class="btn btn-primary btn-sm" :disabled="submitting" @click="previewReissue(batch)">补发</button></div></td>
              </tr>
    ...
    <BaseDialog :show="reissueConfirmOpen" title="确认补发额度" @close="reissueConfirmOpen=false">
      <div v-if="reissuePreview" class="space-y-3 text-sm">
        <p>原始批次：<strong>#{{ reissuePreview.source_batch_id }}</strong>（{{ batchStatusText(reissuePreview.source_status) }}）</p>
        <p>执行方式：确认后立即创建一个独立补发批次</p>
        <p>单用户金额：<strong>{{ money(reissuePreview.amount) }}</strong></p>
        <p>额度到期：<strong>{{ localDate(reissuePreview.expires_at) }}</strong></p>
        <p>资格窗口：[{{ localDate(reissuePreview.qualification_start) }}, {{ localDate(reissuePreview.qualification_end) }})</p>
        <p v-if="reissuePreview.eligibility_policy==='rolling_30d_activity_v1'">API 活跃 {{ reissuePreview.api_active_count }} 人；站内活跃 {{ reissuePreview.site_active_count }} 人；两者命中 {{ reissuePreview.both_active_count }} 人；去重合格 {{ reissuePreview.reference_eligible_count }} 人。</p>
        <p v-else>按原批次资格策略预计合格 {{ reissuePreview.reference_eligible_count }} 人。</p>
        <p>预计补发总额：<strong>{{ money(reissuePreview.estimated_total) }}</strong></p>
        <p v-if="reissuePreview.snapshot_source_batch_id" class="text-gray-500">资格名单复用批次 #{{ reissuePreview.snapshot_source_batch_id }} 的已保存快照。</p>
        <p class="rounded-lg bg-amber-50 p-3 font-medium text-amber-800 dark:bg-amber-950/30 dark:text-amber-200">{{ reissuePreview.disclaimer }}</p>
        <label class="flex items-start gap-2"><input v-model="reissueConfirmed" type="checkbox" class="mt-1" /><span>我已核对原资格窗口、单用户金额和额度到期时间</span></label>
      </div>
      <template #footer><button class="btn btn-secondary" @click="reissueConfirmOpen=false">取消</button><button class="btn btn-primary" :disabled="!reissueConfirmed||submitting" @click="commitReissue">{{ submitting ? '正在提交…' : '确认补发' }}</button></template>
    </BaseDialog>
  </AppLayout>
</template>
...
async function previewReissue(batch:RecurringCreditBatch){if(!current.value)return;submitting.value=true;try{reissuePreview.value=await recurringCreditsAPI.previewRecurringCreditReissue(current.value.id,batch.id);reissueSource.value=batch;reissueConfirmed.value=false;reissueConfirmOpen.value=true}catch(error:any){appStore.showError(error.response?.data?.message||error.message||'补发预览失败')}finally{submitting.value=false}}
async function commitReissue(){if(!current.value||!reissueSource.value||!reissueConfirmed.value)return;submitting.value=true;try{const batch=await recurringCreditsAPI.reissueRecurringCreditBatch(current.value.id,reissueSource.value.id);reissueConfirmOpen.value=false;reissueConfirmed.value=false;selectedBatch.value=null;users.value=[];await Promise.all([loadBatches(1),loadTasks(taskPage.value)]);appStore.showSuccess(`补发批次 #${batch.id} 已创建并进入立即执行队列`)}catch(error:any){appStore.showError(error.response?.data?.message||error.message||'补发失败')}finally{submitting.value=false}}
