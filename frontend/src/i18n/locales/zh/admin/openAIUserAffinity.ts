...
export const openAIUserAffinitySettings = {
  ...
  residentTTL: '常驻偏好续期（秒）',
  residentTTLHint: '成功请求后续期的常驻选号偏好，仅影响新会话的选号优先级，不延长已有会话绑定。',
  conversationActiveTTL: '会话活跃占用（秒）',
  conversationActiveTTLHint: 'Persona 用户容量和活动路由的占用窗口。在途请求受保护；空闲后释放占用，恢复时重新申请原 Persona 的容量，不作废会话身份。',
  conversationIdentityTTL: '会话身份保留（秒）',
  conversationIdentityTTLHint: '绑定和别名的独立滑动保留期，默认 7 天。恢复仍使用原账号、Persona 和 epoch；撤销或 epoch 宽限期结束后不可恢复。',
