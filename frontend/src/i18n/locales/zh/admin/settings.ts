    ...
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          enabled: '启用邀请返利',
          enabledHint: '关闭后用户菜单中的邀请页面入口隐藏、注册时忽略邀请码、新充值不再产生返利。已有返利额度仍可转为可用额度。',
          ...
          creditType: '返利额度类型',
          creditTypePermanent: '永久额度',
          creditTypeLimited: '限时额度',
          creditTypeHint: '用户将可用返利转入额度时，按此类型入账。',
          creditValidityDays: '额度有效期（天）',
          creditValidityDaysHint: '限时额度从返利转入成功时开始计算有效期。',
      ...
      registration: {
        ...
        currentUsers: '当前用户数',
        expectedCapacity: '预期用户容量',
        remainingCapacity: '剩余开放名额',
        unlimited: '不限',
        capacityHint: '达到该用户总数后，普通注册和老用户免邀注册会自动关闭；0 表示不限。有效邀请码始终可绕过此上限。',
        capacityInvalid: '预期用户容量必须是大于或等于 0 的整数',
        additionalSlots: '再开放名额',
        applyAdditionalSlots: '应用到预期容量',
        additionalSlotsInvalid: '再开放名额必须是大于 0 的整数',
        ...
        invitationCodeHint: '开启后，用户需要有效邀请码，或通过已启用的老用户免邀资格检测',
        legacyExemption: '符合条件的老用户免邀请码',
        legacyExemptionHint: '允许运维名单中的合格邮箱免邀请码注册；此路径仍受用户容量上限限制',
        legacyImportStats: '已导入历史邮箱 {historical} 个，其中符合条件 {eligible} 个',
        passwordReset: '忘记密码',
      ...
      defaults: {
        ...
        defaultLimitedCredits: '默认限时额度',
        defaultLimitedCreditsHint: '新用户创建或注册时自动发放，每份额度独立计算有效期',
        addDefaultLimitedCredit: '添加默认限时额度',
        defaultLimitedCreditsEmpty: '未配置默认限时额度。新用户不会自动获得限时额度。',
        defaultLimitedCreditsInvalid: '默认限时额度的金额必须大于 0，有效期必须为 1 至 36500 天。',
        limitedCreditAmount: '额度金额',
        limitedCreditValidityDays: '有效期（天）',
      ...
      gatewayForwarding: {
        ...
        codexEpochRotationTitle: 'Codex Session epoch 轮换',
        codexEpochRotationHint: '全局作用于 session/full 收敛模式。保存后只影响后续新根会话，不会递增现有 epoch、断开旧 WS 或清理连接池。',
        codexEpochRestoreDefaults: '恢复推荐默认值',
        codexEpochMinAge: '最短寿命（小时）',
        codexEpochMinAgeHint: '范围 1–8760；达到后仍需满足空闲门槛。默认 72。',
        codexEpochMaxAge: '最长寿命（小时）',
        codexEpochMaxAgeHint: '范围为最短寿命至 8760；达到后新根会话不再等待空闲。默认 168。',
        codexEpochJitter: '轮换抖动（小时）',
        codexEpochJitterHint: '范围 0–8760；按账号和 Session 槽位确定性计算，0 表示关闭。默认 24。',
        codexEpochIdleGate: '轮换空闲门槛（分钟）',
        codexEpochIdleGateHint: '范围 1–10080；按 Session 槽位统计，HTTP/WS 共享活动状态。默认 120。',
        codexEpochOldGrace: '旧 epoch 宽限期（小时）',
        codexEpochOldGraceHint: '范围 1–8760；只控制旧 Thread/父子绑定保留时间，不决定何时轮换。默认 48。',
        codexEpochEffectiveRange: '当前空闲轮换基础范围：{min}–{minWithJitter} 小时；强制轮换基础范围：{max}–{maxWithJitter} 小时。实际确定值按账号和槽位稳定分布。',
        codexEpochRangeError: 'Codex epoch 参数必须为整数：寿命、抖动和宽限期不超过 8760 小时，空闲门槛为 1–10080 分钟。',
        codexEpochMaxBeforeMinError: 'Codex epoch 最长寿命不能小于最短寿命。',
      ...
      site: {
        ...
        siteWordmarkSuffix: '品牌后缀',
        siteWordmarkSuffixHint: '显示在站点名称后，例如 API 或 Pro',
        ...
        customerServiceGroupNumber: '客服群号',
        customerServiceGroupNumberPlaceholder: '例如：123456789',
        customerServiceGroupNumberHint: '群号和群链接均配置后，登录用户可在页面右上角复制群号并打开客服群。',
        customerServiceGroupLink: '客服群链接',
        customerServiceGroupLinkPlaceholder: 'https://qm.qq.com/q/example',
        customerServiceGroupLinkHint: '仅支持完整的 HTTP(S) 链接；留空或未配置群号时，页头入口不会展示。',
        customerServiceGroupLinkInvalid: '客服群链接必须是完整的 HTTP(S) 地址',
        docUrl: '文档链接',
      ...
      payment: {
        ...
        securityDepositBonusDailyAmount: '每日保证金赠额（USD）',
        securityDepositBonusDailyAmountHint: '北京时间每日 0 点向符合门槛的用户累计发放；0 表示关闭赠额',
        securityDepositBonusCapRatio: '赠额上限比例（%）',
        securityDepositBonusCapRatioHint: '按当前有效保证金人民币元面值乘以比例，得到相同数值的 USD 赠额上限，不做汇率换算；默认 100%，降低比例不会立即压缩已有赠额',
        ...
        securityDepositPenaltyModeHint: '影子模式只记录本应处罚的事件；真实处罚还要求启用保证金执法',
        securityDepositPenaltyOff: '关闭',
        securityDepositPenaltyShadow: '影子记录',
        securityDepositPenaltyEnforce: '真实处罚',
        securityDepositFreezeHours: '用户实付冻结时长（小时）',
        securityDepositMaxRiskMultiplier: '风险倍率上限',
        securityDepositPolicyVersion: '协议版本',
        securityDepositAgreementZh: '中文协议内容（Markdown）',
        securityDepositAgreementEn: '英文协议内容（Markdown）',
        orderTimeout: '订单超时时间',
      ...
      authSourceDefaults: {
        title: '认证来源默认值',
        description: '按注册来源配置新用户默认余额、并发、订阅、限时额度与授权策略。',
        ...
        defaultLimitedCreditsLabel: '默认限时额度',
        defaultLimitedCreditsHint: '仅对当前认证来源生效；每份额度在发放后独立开始计算有效期。',
        noSourceLimitedCredits: '当前来源未配置专属默认限时额度。',
