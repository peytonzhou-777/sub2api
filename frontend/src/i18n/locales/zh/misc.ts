export default {
  ...
  payment: {
    ...
    result: {
      success: '支付成功',
      securityDepositSuccess: '保证金缴纳成功',
    ...
    account: {
      ...
      depletedCreditDetails: '已耗尽限时额度',
      depletedCreditHint: '按额度耗尽时间筛选，单次最多查询 366 天',
      noDepletedCredits: '所选时间范围内暂无已耗尽限时额度',
      depleted: '已用完',
      depletedAt: '{time} 耗尽',
      historyRangeInvalid: '请选择有效且不超过 366 天的时间范围',
      historyLoadFailed: '已耗尽限时额度加载失败，请稍后重试',
      ...
      securityDepositBonusCreditReason: '保证金赠额',
      usage: '额度用量',
      daysRemaining: '剩余 {days} 天',
      expiresToday: '今天到期',
    ...
    securityDeposit: {
      title: '安全保证金',
      separateBalanceHint: '保证金与调用额度完全独立，不会被正常调用扣除。',
      loadFailed: '保证金信息加载失败，请稍后重试。',
      totalBalance: '保证金总额',
      paidBalance: '用户实付余额',
      adminGrantBalance: '管理员发放余额',
      refundable: '当前可退款',
      refundPending: '退款处理中',
      timedLocked: '限时冻结 {amount}',
      permanentLocked: '永久冻结 {amount}',
      nextUnlockAt: '最近解冻时间 {time}',
      ...
      bonusTitle: '保证金赠额',
      bonusDescription: '达到任一可用分组的保证金门槛后，每日赠额会累计到同一份 7 日限时额度并刷新到期日；上限按人民币保证金面值比例计算。',
      bonusCurrent: '当前赠额',
      bonusDaily: '每日配置额',
      bonusCap: '当前上限（{ratio}%）',
      bonusEstimated: '下次预计新增',
      bonusExpiresAt: '当前赠额 {time} 到期',
      bonusEligible: '已达到“{group}”分组门槛，预计于 {time} 新增 {amount}。',
      bonusReasons: {
        enforcement_disabled: '平台当前未启用保证金执法，暂不发放赠额。',
        daily_amount_disabled: '平台当前未配置每日赠额。',
        threshold_not_met: '当前有效保证金尚未达到任一可用分组的非零门槛。',
        cap_reached: '当前赠额已达到按有效保证金计算的上限；符合资格时下一次仍会刷新 7 日有效期。'
      },
      ...
      hideDetails: '收起批次明细',
      paidLot: '实付保证金',
      adminGrantLot: '管理员发放保证金',
      originalAmount: '原始金额 {amount}',
      permanentlyLocked: '永久冻结',
      refundableNow: '可退款',
      contactAdmin: '需联系管理员退款',
      lockedUntil: '冻结至 {time}',
      notRefundable: '当前不可退款',
      refundAction: '申请退款',
      refundTitle: '确认退回保证金',
      refundPrincipal: '退回保证金本金',
      gatewayRefundAmount: '原渠道预计退款',
      affectedKeys: '退款后将自动禁用 {count} 个密钥',
      requiredAfterRefund: '门槛 {amount}',
      refundConfirmationHint: '确认后将先停止新的模型请求并等待在途请求结束，再按原支付渠道退回本批次全部剩余保证金。已自动禁用的密钥不会因退款失败而自动启用。',
      confirmRefund: '确认原路退款',
      refundSucceeded: '保证金退款已完成。',
      refundPendingReview: '退款已提交，正在等待支付渠道确认；系统不会重复发起退款。',
      refundFailed: '保证金退款处理失败，请刷新后重试。'
    },
	refunds: {
	  title: '余额清退',
	  backToAccount: '返回账户',
	  permanentBalance: '剩余永久额度',
	  rechargeBonus: '剩余充值赠额',
	  otherCreditToClear: '其他将清空额度',
	  gatewayTotals: '原路退回金额',
	  orders: '充值订单与退款路由',
	  orderCount: '共 {count} 笔',
	  order: '订单',
	  paid: '原支付金额',
	  bonus: '剩余赠额',
	  eligibleCredit: '参与计算额度',
	  actualRefund: '实际退回',
	  manualReview: '需要人工核验',
	  estimateReady: '当前账户总额已核验，可开始余额清退。',
	  estimateBlocked: '当前账户数据无法自动闭合，暂不能自助清退。',
	  lockAcknowledgement: '我确认开始后账户将立即停止 API 使用；已经在途的请求仍会完成计费，并以排空后的最终报价为准。',
	  finalAcknowledgement: '我已核对最终报价，确认清空全部可退款充值权益及其余限时额度，并按各充值订单原路退回。',
	  start: '开始清退',
	  confirm: '确认余额清退',
	  continue: '继续处理',
	  cancelAndUnlock: '取消清退并解锁账户',
	  reconciling: '正在核验网关退款状态',
	  recoveryHelp: '清退流程尚未结束。可继续处理已明确失败的路由；网关结果未知时请联系客服人工核验，系统不会自动重复退款。',
	  draining: '正在停止 API 并完成最终核算',
	  canceled: '清退已取消，账户状态已恢复',
	  restoreTitle: '恢复退款会话',
	  restoreDescription: '账户已停止 API 使用。验证身份后可继续查看或确认同一笔余额清退，不会恢复普通登录权限。',
	  restore: '验证并继续',
    ...
    errors: {
      ...
      REFUND_FAILED: '退款失败',
	  REFUND_MANUAL_REVIEW: '账户余额需要人工核验',
	  REFUND_QUOTE_CHANGED: '退款金额已变化，请重新核对',
	  REFUND_ALREADY_ACTIVE: '账户已有进行中的清退流程',
	  REFUND_NOT_READY_TO_CONFIRM: '在途计费尚未全部完成',
